...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === '--opencode-root') opts.roots.opencode = next();
    else if (a === '--root') {
      const v = next();
      const eq = v.indexOf('=');
...
function inspectGithubDecision(opts) {
  ...
  if (onlyGithub.length) recommendedActions.push('GitHub contains skills missing locally; ask whether to pull/merge them into Codex, Claude, Antigravity, VSCode, and OpenCode.');
