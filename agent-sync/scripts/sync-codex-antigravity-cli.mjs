...
function parseCodexPath(file) {
  ...
  const setSection = findTomlSection(text, 'shell_environment_policy.set');
  if (section) {
  ...
  if (setSection) {
    const body = text.slice(setSection.headerEnd, setSection.end);
    const nestedPath = findPathValueInBody(body, /^(\s*PATH\s*=\s*)('([^']*(?:''[^']*)*)'|"((?:\\.|[^"\\])*)")/m);
    if (!nestedPath) return { text, pathValue: '', entries: [], hasPath: false, section, setSection };
    const pathValue = parseQuotedTomlString(nestedPath.raw);
    return {
      text,
      pathValue,
      entries: splitPathValue(pathValue),
      ...
      section,
      setSection,
      valueStart: setSection.headerEnd + nestedPath.valueStart,
      valueEnd: setSection.headerEnd + nestedPath.valueEnd,
    };
  }
  if (!section) return { text, pathValue: '', entries: [], hasPath: false, section: null, setSection: null };
  return {
    text,
    pathValue: '',
    entries: [],
...
function updateCodexPath(parsed, file, newValue) {
  ...
  } else if (parsed.setSection) {
    text = text.slice(0, parsed.setSection.headerEnd) + `PATH = ${replacement}\n` + text.slice(parsed.setSection.headerEnd);
