# Desktop Auto-Organizer Script
# Monitors Desktop folder and organizes files automatically

$desktopPath = [Environment]::GetFolderPath("Desktop")

# File categories and their extensions
$categories = @{
    "Desktop-Documents" = @(".pdf", ".doc", ".docx", ".txt", ".xlsx", ".xls", ".pptx", ".ppt", ".odt", ".rtf", ".csv")
    "Desktop-Images" = @(".jpg", ".jpeg", ".png", ".gif", ".bmp", ".svg", ".webp", ".ico", ".tiff", ".psd", ".ai")
    "Desktop-Videos" = @(".mp4", ".avi", ".mkv", ".mov", ".wmv", ".flv", ".webm", ".m4v", ".mpg", ".mpeg")
    "Desktop-Audio" = @(".mp3", ".wav", ".flac", ".aac", ".ogg", ".wma", ".m4a", ".opus")
    "Desktop-Archives" = @(".zip", ".rar", ".7z", ".tar", ".gz", ".bz2", ".iso", ".dmg")
    "Desktop-Installers" = @(".exe", ".msi", ".dmg", ".pkg", ".deb", ".rpm", ".apk")
    "Desktop-Code" = @(".js", ".py", ".java", ".cpp", ".c", ".cs", ".php", ".html", ".css", ".json", ".xml", ".sql", ".sh", ".bat", ".ps1")
    "Desktop-Ebooks" = @(".epub", ".mobi", ".azw", ".azw3")
    "Desktop-Fonts" = @(".ttf", ".otf", ".woff", ".woff2")
}
...
function Organize-ExistingFiles {
    Write-Host "`nOrganizing existing files on Desktop..." -ForegroundColor Yellow
    
    # Get all files on Desktop (exclude folders and shortcuts)
    $files = Get-ChildItem -Path $desktopPath -File | Where-Object { 
        $_.Extension -ne ".lnk" -and $_.Name -notmatch "^Desktop-" 
    }
    ...
    Write-Host "Desktop organized!`n" -ForegroundColor Green
}
...
$action = {
    ...
    # Skip if it's a folder, shortcut, or already in an organized folder
    ...
    # Skip if file is in an organized folder
    if ($fileName -match '^Desktop-') {
        return
    }
    
    # Wait for file to be completely created
...
Write-Host "  DESKTOP AUTO-ORGANIZER RUNNING" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Monitoring: $desktopPath" -ForegroundColor Yellow
