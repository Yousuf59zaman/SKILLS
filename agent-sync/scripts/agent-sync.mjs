...
const SKILLS_VALUE = new Set(['--max-depth', '--mtime-tolerance-ms', '--codex-root', '--claude-root', '--antigravity-root', '--vscode-root', '--opencode-root', '--root', '--github-decision', '--github-repo', '--github-worktree', '--github-source', '--github-branch', '--github-branch-prefix']);
const TOOLS_FLAGS = new Set(['--no-install', '--no-mcp-config']);
const MCP_ONLY_VALUE = new Set(['--claude-config', '--antigravity-mcp', '--vscode-mcp', '--opencode-config', '--startup-timeout']);
